...
optparser.add_option("-i", "--input", dest="input", default="/home/zhelunw/Documents/head500.data", help="File containing sentences to translate (default=data/input)")
...
for (fucking_index, f) in enumerate(french):
  ...
  winners = sorted(stacks[-1].itervalues(), key=lambda h: h.logprob)
  ...
  for winner in winners:
    ...
    return_list = sum_features(winner)
    # print return_list  
    print "%d ||| %s ||| %f %f %f %f " % (fucking_index, extract_english(winner) , return_list[0], return_list[1], return_list[2], return_list[3])
